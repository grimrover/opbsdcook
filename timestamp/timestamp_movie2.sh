...
function convert_mov_to_mp4() {
  ...
  ffmpeg -y -i $1\
    -vf "setdar=16:9,drawtext=fontfile=Verdana.ttf:fontsize=65:expansion=normal:text='%{pts\\:$time\\:$seconds}':r=30:x=(w-tw)-40:y=h-(2*lh):fontcolor=white:box=1:boxcolor=0x000000@1"\
    -preset ultrafast -f mp4 out/$filename.mp4 &>>log
  mv out/*.mp4 ~/local_downs/
}
...
function add_timestamp(){
  ...
  #PTS
  # arg 1 -> format -> flt, hms, gmtime, $time
  # arg 2 -> offset added to pts time

  echo "Adding timestamp $1" &>>log
  ffmpeg -y -i $file\
    -vf "drawtext=fontfile=Verdana.ttf:fontsize=65:expansion=normal:text='%{pts\\:localtime\\:$seconds}':r=30:x=(w-tw)-40:y=h-(2*lh):fontcolor=white:box=1:boxcolor=0x000000@1"\
    -preset ultrafast -f mp4 out/$filename &>>log
  mv out/*.mp4 ~/local_downs/
}
...
echo "" > log
echo "Adding timestamp to MP4 files"
